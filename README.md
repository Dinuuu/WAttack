...
[![Gitter](https://badges.gitter.im/Join%20Chat.svg)](https://gitter.im/Wolox/w_attack?utm_source=badge&utm_medium=badge&utm_campaign=pr-badge&utm_content=badge)
...
- Clone the repository by running `git clone git@github.com:Wolox/w_attack.git`
- Go to the project root by running `cd w_attack`
...
```bash
  ...
  > CREATE ROLE "w_attack" LOGIN CREATEDB PASSWORD 'w_attack';
```
...
Install [Docker Compose](https://docs.docker.com/compose/install/) and then run:
  ...
  ```bash
    > git clone https://github.com/Wolox/w_attack.git
    > docker-compose up
  ```
